...
const parseArray = ({ array = [], keyId = '' }) => {
    Object.fromEntries(array?.map(item => [keyId ? item?.[keyId] : item, item]) || [],
    )
}
...
const newData = filterObject(parseArray({ array: oldData, keyId: 'code' }));
